# refractive_swan_core – Domain core, staging, and mapping types

`refractive_swan_core` keeps the pure domain model for refractive_swan: ServiceRequest/value objects, staging rows, mapping results, and minimal FHIR shapes. It stays free of IO and environment access so app/pipeline layers can inject policy and configuration. Modules are grouped into four super-domains to keep dependencies explicit: `primitives/`, `clinical/`, `interop/`, and `semantics/`.
...
- `prelude.rs` – convenience re-exports so consumers can keep using `refractive_swan_core::{patient, order, staging, mapping, value, fhir}` paths.
...
- Staging structs are shared across `refractive_swan_ingestion::transforms::sr_to_staging`, `refractive_swan_pipeline::bundle_to_mapped_sr`, and the datamart loader; fields mirror the staging tables in the FHIR/NCIt ERDs.
