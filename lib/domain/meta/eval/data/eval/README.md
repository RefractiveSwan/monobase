...
Set `DFPS_EVAL_DATA_ROOT` to override the default (`lib/domain/meta/eval/data/eval/`) when loading datasets. Each NDJSON is paired with `<dataset>.manifest.json`, which records version, license, row count, and a SHA-256 checksum validated by `dfps_eval`.
