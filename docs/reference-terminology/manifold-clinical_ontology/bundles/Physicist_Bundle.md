...
# Physicist Bundle - Mean Field, Geometry, and Random Ensembles
**Date:** 2025-11-15

## Mapping to statistical physics
- Mean-field capacity alpha from support-function statistics resembles margin in random field models.
- Anchors correspond to extreme points; participation ratio D_M parallels inverse participation ratio in disordered systems.
- Centroid correlations rho_CC act like low-rank perturbations to a random ensemble.


**Analogy map**
```mermaid
graph LR
  A[Manifolds] -->|anchors| E[Extremal states]
  B[Participation Ratio D_M] --> F[IPR]
  C[Centroid Corr rho_CC] --> G[Low-rank perturbation]
  W[Width W_M] --> Alpha[Capacity alpha]
```


## Quantities
- D_M = (sum_i lambda_i)^2 / sum_i lambda_i^2 (anchor-projection spectrum).
- Width W_M = R_M * sqrt(D_M) controls alpha in ball approximation.
