...
- `refractive_swan_mapping::MappingEngine` executes the Engine lifeline and assigns
  `MappingState` (`AutoMapped`, `NeedsReview`, `NoMatch`) based on the thresholds
  documented in the architecture page.
- `refractive_swan_pipeline::bundle_to_mapped_sr` provides the orchestration glue between
  staging (`refractive_swan_ingestion`) and mapping, ensuring the sequence stays intact for
  each Bundle.
