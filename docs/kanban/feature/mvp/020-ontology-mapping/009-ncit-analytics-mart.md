...
**Goal:** Materialize an NCIt-aware analytics mart (dim/fact layer) fed by `refractive_swan_pipeline::bundle_to_mapped_sr`, aligned with the NCIt ERD docs.
...
- [ ] Extend `docs/system-design/clinical/ncit/models/data-model-er.md` with an "Implementation" section referencing `refractive_swan_datamart`.
...
- [ ] Confirm adding `refractive_swan_datamart` does not regress existing tests or CI.
...
- [x] Created `lib/app/servers/datamart` crate (`refractive_swan_datamart`).
...
- [x] Provide constructors derived from domain/staging types: `refractive_swan_core::patient::Patient`, `refractive_swan_core::encounter::Encounter`, `refractive_swan_core::staging::StgSrCodeExploded`, and `refractive_swan_core::mapping::DimNCITConcept`.
...
- [x] Added refractive_swan_datamart unit tests that assert the NO_MATCH dimension exists and remains linked to NoMatch facts.
...
- [x] Mentioned `refractive_swan_datamart` in `docs/system-design/clinical/ncit/architecture.md` as the warehouse bridge layer.

---

## Acceptance Criteria
- `refractive_swan_datamart` compiles and is wired into the workspace.
