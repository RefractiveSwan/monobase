...
- [x] Introduce a dedicated eval crate `lib/domain/meta/evaluation` (`refractive_swan_eval`):
  ...
  - [x] Add support for multiple datasets:
    ...
    - Config-driven dataset root (`refractive_swan_EVAL_DATA_ROOT`).
...
- [x] Advanced stats when the feature flag `eval-advanced` is set:
  - [x] Bootstrap confidence intervals for key metrics (`refractive_swan_eval::bootstrap_metrics`).
  - [x] Calibration-style summaries (e.g., score buckets vs correctness).

### EVAL-PLAT-03 – CI & regression gates

- [x] Add a CLI entrypoint:
- `refractive_swan_cli eval-mapping --dataset pet_ct_small --thresholds lib/domain/fake_data/meta/eval_thresholds.json`:
...
- [x] Emit machine-readable summaries (`eval_summary.json` + `eval_results.ndjson`) via `refractive_swan_cli eval_mapping --out-dir <dir>`.
- [x] CLI artifacts: `refractive_swan_cli eval_mapping --out-dir <dir>` writes `eval_summary.json` + `eval_results.ndjson` for CI/dashboards.
- [x] Simple Markdown report via `refractive_swan_cli eval_mapping --report <path>` (temporary CLI-side generator until a richer HTMX/refractive_swan_eval report lands).
- [x] Add a small HTMX/markdown report generator in `refractive_swan_eval` and `refractive_swan_web_frontend`:
  - [x] Renders tables of metrics and a short changelog comparing against a baseline.
- [x] Add endpoints in `refractive_swan_api` to expose latest eval summaries to the web UI (`GET /api/eval/summary?dataset=...`).
...
- [x] Create nine distinct datasets (3× bronze/silver/gold):
  ...
  - Updated `lib/domain/fake_data/eval/README.md` with tier descriptions; datasets consume shared schema under `refractive_swan_EVAL_DATA_ROOT`.
...
### EVAL-PLAT-06 – Migrate/unwrap 012 harness into `refractive_swan_eval`

* [x] Create new crate `lib/domain/meta/evaluation` (`refractive_swan_eval`).
  * [x] Move `EvalCase`, `EvalResult`, `EvalSummary`, `run_eval` out of `refractive_swan_mapping::eval` into `refractive_swan_eval`.
  * [x] Re-export from `refractive_swan_mapping` temporarily to avoid churn (`pub use refractive_swan_eval::*`), then remove once downstream crates are updated.
* [x] Update imports across `refractive_swan_test_suite` to use `refractive_swan_eval`.
* [x] Provide a small, streaming NDJSON reader in `refractive_swan_eval::io` (avoids loading entire datasets into memory).
* [x] Deprecation note in `lib/domain/mapping/src/eval.rs` (with `#[deprecated]`) pointing to `refractive_swan_eval`.

**Acceptance:** `refractive_swan_test_suite::integration::mapping_eval` compiles against `refractive_swan_eval` with no regressions; old module emits a deprecated warning only.
...
* [x] Add a loader in `refractive_swan_eval::datasets` that:
  * [x] Resolves data root via `refractive_swan_EVAL_DATA_ROOT` (falls back to `lib/domain/fake_data/eval`).
...
**Acceptance:** `refractive_swan_cli eval-mapping --dataset pet_ct_small` reads via manifest and prints a warning if the checksum is stale.

### EVAL-PLAT-08 – Reproducibility & determinism guardrails

* [x] Ensure all scoring paths remain deterministic across platforms:
  * [x] Audit hash usage and iteration order in `refractive_swan_mapping` (e.g., `HashSet` -> order-independent usage) and confirm determinism of `VectorRankerMock`.
  * [x] Add `--deterministic` flag to CLI that asserts stable results vs a prior `eval_results.json`.
* [x] Add a property test in `refractive_swan_test_suite`:
...
* [x] Crate: `lib/app/frontend/cli` (`refractive_swan_cli`) with subcommand:
  * [x] `refractive_swan_cli eval-mapping --input lib/domain/fake_data/eval/pet_ct_small.ndjson --thresholds lib/domain/fake_data/meta/eval_thresholds.json --out target/eval/pet_ct_small.json`
...
* [x] In `refractive_swan_web_frontend`:
...
* [x] Add `refractive_swan_eval::report` to emit:
...
- `refractive_swan_eval` can run against multiple gold-standard datasets and emit rich metrics.
