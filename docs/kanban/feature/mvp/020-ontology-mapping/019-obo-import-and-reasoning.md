...
- [x] Add a new crate `lib/domain/ontologies/terminology (obo_graph module)` (`refractive_swan_terminology::obo_graph`) with a focused API for NCIt + one companion ontology (e.g., MONDO) and deterministic parsing.
...
- **Crates & Paths:**
  - `lib/domain/ontologies/terminology (obo_graph module)` (`refractive_swan_terminology::obo_graph`)
  - `data/clinical/ontologies` (fixtures referenced by refractive_swan_terminology::obo_graph)
...
- **Experiments / CI Hooks:**
  - `refractive_swan_terminology::obo_graph` parser/round-trip unit tests
...
- **Experiments / CI Hooks:**
  - Memoization benchmarks in `refractive_swan_terminology::obo_graph`
...
- [x] Extend `refractive_swan_terminology::obo` to bridge `DimNCITConcept.ncit_id` → `OntologyGraph` node IRIs and expose version metadata for downstream caches.

- [x] In `refractive_swan_mapping`:
...
- **Docs & Kanbans Touched:**
  ...
  - `docs/kanban/feature/mvp/019-obo-import-and-reasoning.md`
- **Experiments / CI Hooks:**
  - `refractive_swan_cli map_codes` + `refractive_swan_cli eval_mapping` smoke tests with/without `obo-graph`
  - `refractive_swan_test_suite/tests/integration/vector_mapping.rs` (coverage for synonym expansion)
- **Interfaces & Contracts:**
  - Feature flag: `obo-graph`
  - APIs: terminology bridge helpers; `LexicalRanker` synonym expansion hooks
  - Env: `refractive_swan_EVAL_DATA_ROOT` for eval datasets when testing effects
...
- [x] Tests in `refractive_swan_terminology::obo_graph`:
...
- [x] Mapping tests in `refractive_swan_test_suite`:
...
- **Interfaces & Contracts:**
  - CLI/data: fixtures consumed by `refractive_swan_cli map_codes`/`eval_mapping` smoke tests
...
- **Experiments / CI Hooks:**
  ...
  - Doc examples synced with `refractive_swan_cli`/`refractive_swan_terminology::obo_graph` APIs
- **Interfaces & Contracts:**
  - Documentation links for `obo-graph` feature flag and `OntologyGraph` API
  - References to `refractive_swan_cli map_codes`/`eval_mapping` usage with OBO context
...
- Supporting arbitrary OBO ontologies beyond the curated set used by refractive_swan.
