...
- [x] Define environment profile taxonomy (`dev`, `test`, `prod`) plus override rules (`refractive_swan_ENV_FILE`, `refractive_swan_WORKSPACE_ROOT`).
...
- Added `refractive_swan_configuration` crate that discovers the workspace root, resolves `refractive_swan_ENV` / `APP_ENV`, and auto-loads `.env.<namespace>.<profile>` files for every surface.
- Wired `refractive_swan_api`, `refractive_swan_web_frontend`, `refractive_swan_cli`, `refractive_swan_eval` (fake data generators), `refractive_swan_observability`, and `refractive_swan_test_suite` into the loader so backend, UI, CLIs, and tests all consume the right env files without manual sourcing.
- Expanded `docs/runbook/web-quickstart.md` with the multi-env story, namespace examples, storage/secrets guidance, and `refractive_swan_ENV_STRICT` expectations for CI.
