...
- [x] Implement `UmlsTerminologyClient` (behind feature flag `umls-http`):

  - [x] Uses `reqwest` to call configured UMLS / NCIm endpoints.
  - [x] Handles auth headers, rate limits (backoff), and simple pagination (minimal scaffold).

- [x] Implement `NcitTerminologyClient` (feature `ncit-http`) if distinct:

  - [x] Resolve NCIt codes and synonyms.
  - [x] Align responses with `NcitRecord`.
