...
### DESK-01 – Scaffold desktop shell crate (`refractive_swan_desktop`)
- [ ] Create `code/lib/app/frontend/desktop/refractive_swan_desktop` with `Cargo.toml` + `src/main.rs`.
...
- [ ] Add dependency on `refractive_swan_pipeline`, `refractive_swan_eval::fake_data`, and `refractive_swan_observability`.
- [ ] Implement a simple “Load & map sample bundle” flow:
  - [ ] Option A: load the baseline bundle fixture from `refractive_swan_test_suite::regression::baseline_fhir_bundle`.
...
- [ ] Reuse `refractive_swan_observability::PipelineMetrics` to show per-run metrics in the UI.
...
- [ ] Update `docs/system-design/base/directory-architecture.md` to list `refractive_swan_desktop` explicitly under `lib/app/frontend/desktop`.
...
- [ ] Ensure doc comments in `refractive_swan_desktop` link to `docs/system-design/clinical/fhir/*` and `docs/system-design/clinical/ncit/*` where appropriate.
