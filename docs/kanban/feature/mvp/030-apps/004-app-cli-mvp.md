...
### APP-01 – Scaffold CLI app crate (`refractive_swan_cli`)
- [ ] Create `code/lib/app/frontend/cli/refractive_swan_cli` crate with `src/main.rs`.
...
- [ ] Implement a `map-bundles` subcommand that:
  ...
  - [ ] Calls `refractive_swan_pipeline::bundle_to_mapped_sr` for each bundle.
  - [ ] Writes NDJSON rows for `StgServiceRequestFlat`, `MappingResult`, and `DimNCITConcept`.
- [ ] Wire in `refractive_swan_observability::PipelineMetrics` so each run emits a final metrics summary line.
...
- [ ] Implement a `generate-fhir-bundles` subcommand that wraps `refractive_swan_eval::fake_data::raw_fhir` helpers.
...
- [ ] Initialize logging via `env_logger` and respect `RUST_LOG` for `refractive_swan_pipeline` and `refractive_swan_mapping`.
...
- [ ] Update `docs/system-design/base/directory-architecture.md` to:
  ...
  - [ ] Explicitly list `code/lib/app/frontend/cli/refractive_swan_cli` under the `app/` bucket with responsibilities.
...
- [ ] Update `docs/system-design/clinical/fhir/index.md` Quickstart CLI snippets to use `refractive_swan_cli` subcommands instead of ad-hoc binaries.

### APP-06 – Tests & CI for CLI surfaces
- [ ] Add integration tests in `refractive_swan_test_suite` that exercise the CLI `run()` entrypoint with:
...
- [ ] Optionally add a small CLI smoke test in CI (e.g., run `refractive_swan_cli generate-fhir-bundles | refractive_swan_cli map-bundles` on 1–2 bundles and assert exit code 0).
