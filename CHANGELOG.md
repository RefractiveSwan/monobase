...
- REFR-14 – `refractive_swan_test_suite::ensure_eval_data_root` now validates overrides, tolerates parent `.../data` directories, and falls back to the bundled fixtures when the override path is missing, with the setup/env/eval runbooks documenting the behavior so `cargo make test` works out of the box.
